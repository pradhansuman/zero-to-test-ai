...
- Name: auto-generated.spec.ts >> 🚀 COMPREHENSIVE TEST SUITE - AUTO-GENERATED >> Account Pages >> Addresses link navigates correctly
- Location: tests/e2e/auto-generated.spec.ts:722:9

# Error details

```
Error: locator.count: Unexpected token "/" while parsing css selector "a:has-text(/Addresses/i), a[href*="customeraddresses"]". Did you mean to CSS.escape it?
```
...
> 725 |       if (await linkElement.count() > 0) {
  ...
  736 | 
  737 |   // ═════════════════════════════════════════════════════════
  738 |   // FORM VALIDATION - All Fields (15+ tests)
  739 |   // ═════════════════════════════════════════════════════════
  740 | 
  741 | 
  742 |   test.describe('Form Validation - Registration', () => {
  743 |     test('Email field validation - required', async ({ page }) => {
  744 |       await page.goto(`${BASE_URL}/register`);
  745 |       const emailField = page.locator('input[type="email"]').first();
  746 |       if (await emailField.count() > 0) {
